...
class ClassroomSession(BaseEntity):
    ...
    frame_logs: Mapped[List["FrameLog"]] = relationship(
        back_populates="session", cascade="all, delete-orphan"
    )
    classroom_metrics: Mapped[List["ClassroomMetric"]] = relationship(
        back_populates="session", cascade="all, delete-orphan"
    )
    seatings: Mapped[List["SessionSeating"]] = relationship(
        back_populates="session", cascade="all, delete-orphan"
    )
    student_metrics: Mapped[List["StudentMetric"]] = relationship(
        back_populates="session", cascade="all, delete-orphan"
    )
